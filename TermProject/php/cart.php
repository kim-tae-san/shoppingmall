...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/TermProject/css/cart.css">
</head> 
<body>
    <!-- top header -->
        ...
        <form method="post" action="index.php"> 
            <div class="cartlist">
                ...
                while($row = mysqli_fetch_array($query)){
                    ...
                    $prodinfo = mysqli_fetch_array($query2);
                    $total += $prodinfo['price'] * $row['productcount'];
                    $n++;
                    $cartid = $row['id'];
                    $goodsid = $prodinfo['id'];
                ?>
                <div class="cartitem">
                    <input type="button" value="X" id="delete" onclick="location.href='itemdelprocess.php?cartid=<?=$cartid?>'">
                    <img style="cursor: pointer;" onclick="location.href='goods_view.php?goodsid=<?=$goodsid?>'" src=<?=$prodinfo['imgurl']?>></td>
                    <div class="iteminfo">
                        <span style="cursor:pointer;" onclick="location.href='goods_view.php?goodsid=<?=$goodsid?>'"id="itemname"><?=$prodinfo['name']?></span>
                        <br><br>
                        <?= $row['productcount']?>개
                        <br><br>
                        <?=$prodinfo['price']*$row['productcount']?>원
                    </div>
                </div>
                <?php
            }
            ?>
            <br>
            <u onclick="location.href='goods_list.php'">< 쇼핑 계속하기</u>
            <div class="totalinfo">
                <?php
                    if($total<15000)
                        $delive = 2500;
                    if($n==0)
                        $delive = 0;
                ?>
                <div>합계<br><span style="font-weight: 800;"><?=$delive+$total?>원<span></div>
                <span id="dot2">=</span>
                <div> 배송비<br><span style="font-weight:800;"><?=$delive?></span></div>
                <span id="dot1">+</span>
                <div> 총 <span style="font-weight:800;"><?=$n?></span> 개의 상품금액<br><span style="font-weight: 800;"><?=$total?>원</span></div>
            </div>
            <?php
                if($n>0){
                    ?>
            <div style="display: flex; flex-direction: row-reverse; padding-top: 30px;" onclick="alert('상품이 주문되었습니다.'); location.href='index.php';">
                <input style="cursor: pointer;"type="submit" value="주문" id="submit"></input> 
            </div>
            <?php
        }
        ?>
